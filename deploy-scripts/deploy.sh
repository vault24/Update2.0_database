...
# ===========================================================================
# 3. BACKEND  (.env, venv, deps, backup, migrate, static, superuser)
# ===========================================================================
write_backend_env() {
  local env_file="${SERVER_DIR}/.env"
  local from_email="SIPI Management System <${EMAIL_HOST_USER:-noreply@${STUDENT_DOMAIN:-localhost}}>"
...
backup_database() {
  # Safety net before migrations. Best-effort: never blocks the deploy.
  mkdir -p "${BACKUP_DIR}"
  local stamp; stamp="$(date +%Y%m%d_%H%M%S)"
  if sudo -u postgres pg_dump "${DB_NAME}" 2>/dev/null | gzip > "${BACKUP_DIR}/${DB_NAME}_${stamp}.sql.gz"; then
    ok "Database backed up: ${BACKUP_DIR}/${DB_NAME}_${stamp}.sql.gz"
    # Keep the 14 most recent backups.
    ls -t "${BACKUP_DIR}/${DB_NAME}"_*.sql.gz 2>/dev/null | tail -n +15 | xargs -r rm
  else
    warn "Database backup skipped (empty database or pg_dump failed)"
  fi
}
