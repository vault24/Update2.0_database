...
/** Label + required marker + helper/error text wrapper used by every field. */
export function FieldWrapper({ label, required, helper, error, htmlFor, className, children }: FieldWrapperProps) {
  return (
...
interface TextFieldProps {
  ...
  maxLength?: number;
  /** Restrict input to digits only (strips anything non-numeric as you type). */
  numeric?: boolean;
}

export function TextField({
  label, value, onChange, placeholder, type = 'text',
  required, helper, error, id, inputMode, maxLength, numeric,
}: TextFieldProps) {
  const handleChange = (raw: string) => onChange(numeric ? raw.replace(/\D/g, '') : raw);
  return (
    <FieldWrapper label={label} required={required} helper={helper} error={error} htmlFor={id}>
      <Input
        id={id}
        type={type}
        value={value}
        ...
        onKeyDown={
          numeric
            ? (e) => {
                // Block the common non-digit chars that survive type="text".
...
export function TextareaField({ label, value, onChange, placeholder, required, helper, error, rows = 3 }: TextareaFieldProps) {
  return (
...
/** A subtle grouped section used to break a step into digestible blocks. */
...
/** Step intro heading shown at the top of each step's body. */
export function StepIntro({ icon: Icon, title, description }: { icon?: React.ElementType; title: string; description?: string }) {
  return (
    <div className="flex items-center gap-3">
      ...
      <div>
        <h3 className="font-display text-lg font-bold leading-tight md:text-xl">{title}</h3>
