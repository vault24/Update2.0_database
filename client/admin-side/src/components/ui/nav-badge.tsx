...
export function NavBadge({ count, dot, className }: NavBadgeProps) {
  ...
  if (dot) {
    return (
      <span
        ...
        className={cn(
          'absolute right-1.5 top-1.5 h-2 w-2 rounded-full bg-destructive ring-2 ring-sidebar',
          className,
        )}
      />
    );
  ...
  return (
    <span
      aria-label={`${count} new`}
      className={cn(
        'ml-auto inline-flex h-5 min-w-[20px] items-center justify-center rounded-full bg-destructive px-1.5',
