...
├── 📄 ENVIRONMENT_SETUP.md             ← Environment/config reference
│
├── 📁 deploy-scripts/                  ← Automated deployment
│   ├── config.env                      ← Edit this: server IP, DB, ports…
│   ├── deploy.sh                       ← One-shot idempotent deploy
│   ├── slms.sh / maintenance.sh / troubleshoot.sh
│
├── 📁 server/                          ← Django backend
│   ├── manage.py
│   ├── requirements.txt                ← Production deps
│   ├── requirements-dev.txt            ← Dev/test tooling
│   ├── .env                            ← Backend secrets (not in git; generated by deploy.sh)
...
See `PRODUCTION_GUIDE.md` for the full step-by-step deployment walkthrough, and
`deploy-scripts/` for the automated one-shot deploy.

**Quick redeploy after a code push:**
```bash
cd deploy-scripts && sudo ./deploy.sh --backend && sudo ./deploy.sh --frontend
```
...
`deploy-scripts/deploy.sh` creates the database and user automatically. To do it
manually:

```bash
sudo -u postgres psql -c "CREATE USER sipi_web WITH PASSWORD 'sipiadmin';"
sudo -u postgres psql -c "CREATE DATABASE sipi_db OWNER sipi_web;"
sudo -u postgres psql -c "GRANT ALL PRIVILEGES ON DATABASE sipi_db TO sipi_web;"
```
